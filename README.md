...
| Local MCP stdio resources | Implemented, narrowly bounded | MCP 2026-07-28 `resources/list` and exact `resources/read` only, owner-only ActorContext bootstrap project admission, tenant/actor runtime-principal DB binding, private zero-TTL cache hints, and no write/approval/raw-ledger surface |
