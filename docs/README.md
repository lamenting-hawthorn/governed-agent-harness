# Documentation Index

This directory is the source of truth for the Governed Agent Harness design.
All links below resolve within this fresh repository.

## Product and architecture

- [Vision and scope](VISION_AND_SCOPE.md)
- [Product requirements](PRODUCT_REQUIREMENTS.md)
- [Architecture](ARCHITECTURE.md)
- [Contracts](CONTRACTS.md)
- [Integrations](INTEGRATIONS.md)
- [Memory and knowledge](MEMORY_AND_KNOWLEDGE.md)
- [Governed GitHub Markdown knowledge](GITHUB_MARKDOWN_KNOWLEDGE.md)
- [Local-first MCP roadmap](LOCAL_FIRST_MCP_ROADMAP.md)
- [Skills](SKILLS.md)
- [Evaluation and learning](EVALUATION_AND_LEARNING.md)

## Trust and operations

- [Security model](SECURITY_MODEL.md)
- [Threat model](THREAT_MODEL.md)
- [Data governance](DATA_GOVERNANCE.md)
- [Observability and operations](OBSERVABILITY_AND_OPERATIONS.md)
- [Enterprise readiness](ENTERPRISE_READINESS.md)

## Delivery

- [Developer experience](DEVELOPER_EXPERIENCE.md)
- [Testing strategy](TESTING_STRATEGY.md)
- [Definition of done](DEFINITION_OF_DONE.md)
- [Release strategy](RELEASE_STRATEGY.md)
- [Provenance and clean-room boundaries](PROVENANCE.md)

## Architecture decisions

Architecture decision records live in [`docs/adr/`](adr/).
